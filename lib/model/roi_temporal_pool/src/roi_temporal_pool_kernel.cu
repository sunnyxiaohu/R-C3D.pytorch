...
__global__ void ROIPoolForward(const int nthreads, const scalar_t *bottom_data,
                               const scalar_t *rois,
                               const scalar_t temporal_scale, const int channels,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
	...
	int pl = (index / pooled_w / pooled_h) % pooled_l;
    int c = (index / pooled_w / pooled_h / pooled_l) % channels;
    int n = index / pooled_w / pooled_h / pooled_l / channels;
    ...
    scalar_t roi_x1 = 0;
    scalar_t roi_y1 = 0;
    //scalar_t roi_x2 = (offset_rois[3] + 1) * temporal_scale;
    //scalar_t roi_y2 = (offset_rois[4] + 1) * temporal_scale;
    scalar_t roi_l1 = (offset_rois[1] + 1) * temporal_scale;
    scalar_t roi_l2 = (offset_rois[2] + 1) * temporal_scale;
	
    // force malformed rois to be 1x1
    scalar_t roi_w = width; 
    scalar_t roi_h = height;
	scalar_t roi_l = roi_l2 - roi_l1;
    if (roi_w <= 0 || roi_h <= 0 || roi_l <= 0) continue;

    scalar_t bin_size_w = roi_w / static_cast<scalar_t>(pooled_w);
    scalar_t bin_size_h = roi_h / static_cast<scalar_t>(pooled_h);
	scalar_t bin_size_l = roi_l / static_cast<scalar_t>(pooled_l);
	...
	int bin_l1 = floor(static_cast<scalar_t>(pl) * bin_size_l + roi_l1);
	int bin_l2 = ceil(static_cast<scalar_t>(pl + 1) * bin_size_l + roi_l1);
	...
	bin_l1 = min(max(bin_l1, 0), length);
	bin_l2 = min(max(bin_l2, 0), length);
    bool is_empty = (bin_y2 <= bin_y1) || (bin_x2 <= bin_x1) || (bin_l2 <= bin_l1);
    ...
    scalar_t max_val = is_empty ? static_cast<scalar_t>(0)
                                : bottom_data[bin_l1* height * width + bin_y1 * width + bin_x1] - 1;

	for (int l = bin_l1; l < bin_l2; ++l) {								
		for (int h = bin_y1; h < bin_y2; ++h) {
		  for (int w = bin_x1; w < bin_x2; ++w) {
			int offset = l * height * width + h * width + w;
...
int ROIPoolForwardLaucher(const at::Tensor features, const at::Tensor rois,
  ...
  const int output_size = num_rois * channels * pooled_l * pooled_h * pooled_w;
      ...
      features.type(), "ROIPoolLaucherForward", ([&] {
        ...
        ROIPoolForward<scalar_t>
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                output_size, bottom_data, rois_data, scalar_t(temporal_scale),
                channels, length, height, width, pooled_l, pooled_h, pooled_w, top_data,
                argmax_data);
      }));
...
__global__ void ROIPoolBackward(const int nthreads, const scalar_t *top_diff,
                                ...
                                const scalar_t temporal_scale,
                                const int channels, const int length, const int height,
                                const int width, const int pooled_l, const int pooled_h,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    int bottom_index = argmax_data[(n * channels + c) * pooled_l * pooled_h * pooled_w +
                                   pl * pooled_h * pooled_w + ph * pooled_w + pw];

    atomicAdd(bottom_diff + (roi_batch_ind * channels + c) * length * height * width +
                  bottom_index,
              top_diff[index]);
  }
}
...
int ROIPoolBackwardLaucher(const at::Tensor top_grad, const at::Tensor rois,
  ...
  const int output_size = num_rois * pooled_l * pooled_h * pooled_w * channels;
      ...
      top_grad.type(), "ROIPoolLaucherBackward", ([&] {
        ...
        ROIPoolBackward<scalar_t>
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                ...
                scalar_t(temporal_scale), channels, length, height, width, pooled_l, pooled_h,
